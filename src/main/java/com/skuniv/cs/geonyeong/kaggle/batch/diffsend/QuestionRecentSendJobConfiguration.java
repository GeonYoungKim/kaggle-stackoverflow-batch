...
public class QuestionRecentSendJobConfiguration extends AbstractRecentConfig {
    ...
    @Bean
    @StepScope
    public MultiResourceItemReader<AvroQuestion> multiResourceQuestionRecentItemReader(
        @Value("#{jobParameters[questionRecentPath]}") String questionRecentPath) {
        MultiResourceItemReader<AvroQuestion> multiResourceItemReader = BatchUtil
            .createMultiResourceItemReader(questionRecentPath);
    ...
    public FlatFileItemReader<AvroQuestion> questionRecent() {
        ...
        reader.setLineMapper((line, lineNumber) -> {
            ...
            List<AvroComment> avroCommentList = createAvroCommentList(
                questionSplit[19].split(HIVE_DELEMETER_SECOND, -1));
            List<AvroLink> avroLinkList = createAvroLinkList(
                questionSplit[20].split(HIVE_DELEMETER_SECOND, -1));
            ...
            return AvroQuestion.newBuilder()
                ...
                .setTitle(
                    StringUtils.equals(EMPTY_FIELD_VALUE, questionSplit[1]) ? "" : questionSplit[1])
                .setBody(
                    StringUtils.equals(EMPTY_FIELD_VALUE, questionSplit[2]) ? "" : questionSplit[2])
                .setAnswerCount(StringUtils.equals(EMPTY_FIELD_VALUE, questionSplit[3]) ? 0
                    : Integer.valueOf(questionSplit[3]))
                .setCommentCount(StringUtils.equals(EMPTY_FIELD_VALUE, questionSplit[4]) ? 0
                    : Integer.valueOf(questionSplit[4]))
                .setCreateDate(StringUtils.equals(EMPTY_FIELD_VALUE, questionSplit[5]) ? TimeUtil
                    .toStr(new Date()) : TimeUtil.toStr(questionSplit[5]))
                .setFavoriteCount(StringUtils.equals(EMPTY_FIELD_VALUE, questionSplit[6]) ? 0
                    : Integer.valueOf(questionSplit[6]))
                .setScore(StringUtils.equals(EMPTY_FIELD_VALUE, questionSplit[7]) ? 0
                    : Integer.valueOf(questionSplit[7]))
                .setTags(
                    StringUtils.equals(EMPTY_FIELD_VALUE, questionSplit[8]) ? "" : questionSplit[8])
                .setViewCount(StringUtils.equals(EMPTY_FIELD_VALUE, questionSplit[9]) ? 0
    ...
    public ItemWriter<AvroQuestion> itemQuestionRecentWriter() {
        ItemWriter<AvroQuestion> itemWriter = items -> items.forEach(item -> {
            kafkaProducer.send(
                new ProducerRecord<String, AvroQuestion>(KafkaTopicType.QUESTION_RECENT.name(),
                    item.getId(), item));
            try {
                Thread.sleep(10l);
